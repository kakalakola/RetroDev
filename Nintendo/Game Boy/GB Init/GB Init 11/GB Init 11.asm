;----------------------------------------------------
;GameBoy ASM Project 0.11 - 2020.11.10
;Spawning and despawning a bullet
;by Saad Azim
;----------------------------------------------------
...
_BULLET_DATA:          equ $c108 ;The bullet IS technically a sprite, but still...
_SPRITE_01_DATA:       equ $c10c
...
;Variables needed for bullets. The _BULLET_FLAG is used to make sure only one bullet is spawned. TWO extra bytes are needed to keep track of the initialization vector, which depends on the d-pad input, as well as the actual vector, which is unaffected.
_BULLET_FLAG:          equ $87
_BULLET_VECTOR_Y_INIT: equ $88
_BULLET_VECTOR_X_INIT: equ $89
_BULLET_VECTOR_Y:      equ $8a
_BULLET_VECTOR_X:      equ $8b
...
cpu gbz80
  ...
  db "GB INIT 0.11   "      ;Game Title 15 bytes                                    $0134-$0142
...
loopCopyDMARoutine:
  ...
  ld hl,sprites
  ld bc,spritesEnd-sprites  ;Automatically calculated now
  ld de,_SPRITE_DATA
  call blockCopy
...
loopDecompressMapData:
  ...
  ;Initialize bullet vectors to shoot right, since the sprite is facing right
  xor a
  ;ldh (_BULLET_VECTOR_Y),a
  ldh (_BULLET_VECTOR_Y_INIT),a
  ld a,$02
  ;ldh (_BULLET_VECTOR_X),a
  ldh (_BULLET_VECTOR_X_INIT),a
...
MainLoop:
  ...
  ;Change BG color, and despawn bullet if A is pressed
  ld a,_BG_COLOR
  bit 0,b
  jr nz,skipPalette

  ;Despawn bullet if it's on screen
  xor a
  ldh (_BULLET_FLAG),a
  ldh (_BULLET_VECTOR_Y),a
  ldh (_BULLET_VECTOR_X),a
  ...
  ;Load alternate BG color AFTER bullet has been despawned
  ld a,_BG_COLOR_ALT

skipPalette:
  ldh (_PALETTE_BG),a

  ;Check if Button B is pressed, if not, jump to checkDPad
  bit 1,b                   
  jr nz,checkDPad
  ...
  ;Check if a bullet has already been spawned, by checking the sprite tile index. Spawn a sprite, only if one doesn't exit.
  ld a,(_BULLET_DATA+2)
  cp 0
  jr nz,checkDPad

  ;If Button B is pressed, and no bullet has been spawned, AND there is no sound playing set the spawn bullet flag.
  set 0,a                   ;Set bit 0 to 1 in A
  ldh (_BULLET_FLAG),a      ;Save bullet flag
...
checkDPad:
  ld a,b                    ;Copy B to A, which still contains the raw input flags
  cpl                       ;ComPLement A, effectively setting 1 for an input and 0 for no input
  and a,$f0                 ;Mask out all the non directional bits
  jr z,spawnBullet          ;Jump if bits 7-4 are 0 (i.e. no directional inuput is pressed) to spawnBullet


  ;If the d-pad HAS been pressed, the initialization motion vectors for the bullet need to be reset
  xor a
  ldh (_BULLET_VECTOR_Y_INIT),a
  ldh (_BULLET_VECTOR_X_INIT),a
...
;checkRight:
  ...
  ldh (_MOTION_VECTOR_X),a
  ld a,2                   ;The bullet should move slightly faster than the BG
  ldh (_BULLET_VECTOR_X_INIT),a

  ;Update meta sprite 0 X flip, and tiles, modified due to new arrangement of tiles
  ld a,(_SPRITE_DATA+3)
  res 5,a
  ld (_SPRITE_DATA+3),a
  ld (_SPRITE_DATA+7),a
...
spawnBullet:
  ldh a,(_BULLET_FLAG)
  bit 0,a                   ;If bit 0 in _BULLET_FLAG is 0, the sprite has probably been spawned
                            ;All that needs to be done is to process said sprite
  jr z,processBullet

  res 0,a                   ;Otherwise set bit 0 and store it in HRAM
  ldh (_BULLET_FLAG),a
  ...
  ld a,$54
  ld (_BULLET_DATA+1),a
  ld a,$08                  ;Add another sprite to be copied to OAM
  ld (_BULLET_DATA+2),a

  ld a,(_SPRITE_DATA+3)
  ld (_BULLET_DATA+3),a

  ldh a,(_BULLET_VECTOR_Y_INIT)
  ldh (_BULLET_VECTOR_Y),a
  ldh a,(_BULLET_VECTOR_X_INIT)
  ldh (_BULLET_VECTOR_X),a

processBullet:
  ld hl,(_BULLET_DATA)      ;Load address of Y position of bullet
  ldh a,(_BULLET_VECTOR_Y)  ;Load Y vector of bullet
  ld b,(hl)                 ;Load Y position of bullet
  add a,b                   ;Update Y position of bullet

  cp $08
  jr c,despawnBullet        ;Despawn bullet the bullet Y is less than $08

  ;Where the Zero flag is used to check if two values are equal or not, the Carry flag is used to check if a value is greater or less than another value

  cp $90
  jr nc,despawnBullet       ;Despawn bullet if the bullet Y is more than $A0

  ldi (hl),a                ;Store Y position of bullet in RAM otherwise

  ldh a,(_BULLET_VECTOR_X)  ;Load X vector of bullet
  ld b,(hl)                 ;Load X position of bullet
  add a,b                   ;Update X position of bullet

  cp 0                      ;Check if bullet X is at the left boundary
  jr c,despawnBullet
  cp $a8                    ;Check if bullet X is at the right boundary
  jp nc,despawnBullet

  ldi (hl),a                ;Store X position of bullet in RAM
...
processVectors:
  ...
  ;Update sprites
  ld hl,_BULLET_DATA        ;Load address of bullet data, since the camera movement needs
                            ;to be taken into account regarding where the bullet is seen on screen
  ld d,$07                  ;7 hardware sprites to update, 1 bullet, and 3 meta sprites
...
sprites:
  ...
  db $50, $50,  $0c, %00000000 ;E, left half, using palette 0
  db $50, $58,  $0e, %00000000 ;E, right half, using palette 0
  ...
  db $00, $00,  $00, %00000000 ;Reserved sprite for bullet, using palette 0

  db $43, $37,  $0c, %00010000 ;E, using palette 1
  db $43, $3f,  $0e, %00010000 ;E, using palette 1
  db $69, $45,  $10, %00010000 ;F, using palette 1
  db $69, $4d,  $12, %00010000 ;F, using palette 1
  db $48, $6a,  $12, %00110000 ;F, using palette 1, X flip
  db $48, $72,  $10, %00110000 ;F, using palette 1, X flip
spritesEnd:
...
tileMap:                  ;RLE compression: byte count, byte
  db $10,2,$10,3,$10,2,$10,3,$10,2,$10,3,$10,2,$10,3
  db $10,2,$10,3,$10,2,$10,3,$10,2,$10,3,$10,2,$10,3
  db $10,2,$10,3,$10,2,$10,3,$10,2,$10,3,$10,2,$10,3
  db $10,2,$10,3,$10,2,$10,3,$10,2,$10,3,$10,2,$10,3
  db $10,4,$10,5,$10,4,$10,5,$10,4,$10,5,$10,4,$10,5
  db $10,4,$10,5,$10,4,$10,5,$10,4,$10,5,$10,4,$10,5
  db $10,4,$10,5,$10,4,$10,5,$10,4,$10,5,$10,4,$10,5
  db $10,4,$10,5,$10,4,$10,5,$10,4,$10,5,$10,4,$10,5
tileMapEnd:
